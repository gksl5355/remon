...
from app.crawler.california_law import CaliforniaLawCrawler
...
async def main():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            result = await db.execute(text("SELECT 1 FROM data_sources WHERE source_id = 2"))
            if not result.scalar():
                await db.execute(text("INSERT INTO data_sources (source_id, source_name, url, source_type) VALUES (2, 'CA Legislature', 'https://leginfo.legislature.ca.gov', 'html')"))
            await db.commit()
    ...
    # 3. 크롤러 초기화 및 실행
    crawler = CaliforniaLawCrawler() # FDA 대신 이거 사용
    print("🚀 크롤링 작업을 시작합니다...")
