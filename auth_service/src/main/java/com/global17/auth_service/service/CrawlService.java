...
//     // [공통] 실제 크롤링 로직 (기존 runCrawling 코드를 여기로 옮김)
//     private void processCrawling(String country, String countryCode, String keyword, String category) {
//         // 1. Tavily 검색
//         // category가 news면 PDF 필터 해제, regulation이면 PDF 필터 적용
//         String query = keyword;
//         if ("regulation".equalsIgnoreCase(category)) {
//              query += " filetype:pdf";
//         }
...
//                 if (fileContent != null && fileContent.length > 0) {
//                     String fileName = UUID.randomUUID().toString() + (url.endsWith(".pdf") ? ".pdf" : ".txt");
                    
//                     // 3. S3 업로드 (category 폴더 사용)
//                     String s3Path = s3Uploader.uploadFile(fileContent, fileName, category);

//                     // 4. DB 저장
//                     if (s3Path != null) {
//                         // 중복 체크 로직 등을 추가할 수 있음
//                         Regulation regulation = Regulation.builder()
//                                 .countryCode(countryCode)
//                                 .title(title)
//                                 .sourceId(99)
...
//     public void runCrawling(String country, String countryCode, String keyword) {
//         System.out.println("🚀 [" + country + "] 크롤링 시작: " + keyword);

//         // 1. Tavily 검색
//         String query = country + " " + keyword + " filetype:pdf";
//         List<Map<String, String>> searchResults = tavilyClient.search(query);
...
//             try {
//                 // 2. 파일 다운로드 (바이너리)
//                 System.out.println("⬇️ 다운로드 시도: " + url);
//                 byte[] fileContent = restTemplate.getForObject(url, byte[].class);
...
//                     // 4. S3 업로드 (regulation 폴더)
//                     String s3Path = s3Uploader.uploadFile(fileContent, fileName, "regulation");
...
//                                 .countryCode(countryCode)
//                                 .title(title)
//                                 .sourceId(99) // Discovery Agent ID
//                                 .status("active")
//                                 .createdAt(LocalDate.now())
//                                 .build();
                        
//                         regulationRepository.save(regulation);
//                         System.out.println("✅ DB 저장 완료: " + title);
//                     }
//                 }
//             } catch (Exception e) {
//                 System.err.println("❌ 처리 실패 (" + url + "): " + e.getMessage());
//             }
//         }
//         System.out.println("🎉 [" + country + "] 크롤링 종료");
//     }
// }
...
//     public void runCrawling(String country, String keyword) {
//         // 1. Tavily 검색
//         String query = country + " " + keyword + " filetype:pdf";
//         List<Map<String, String>> searchResults = tavilyClient.search(query);
...
//                 }
//             } catch (Exception e) {
//                 System.out.println("❌ 다운로드 실패: " + url);
//             }
//         }
//     }
// }