...
class AIService:
    ...
    async def restore_state_from_db(
            ...
            preprocess_results=[
                {
                    "regulation_id": regulation_id,
                    "status": "success",
                    "vision_extraction_result": regulation.get(
        ...
        if "change_detection" in intermediate_data:
            ...
            state["change_detection_results"] = cd_data.get(
            ...
            state["regulation_analysis_hints"] = cd_data.get(
    ...
    async def run_pipeline_with_hitl(
        self, db: AsyncSession, regulation_id: int, user_message: str, target_node: str
    ) -> Dict[str, Any]:
        """
        ...
        for i, pdf_path in enumerate(pdf_paths, 1):
            try:
                ...
                if report and report.get("report_id"):
                    ...
                    logger.info(
                        f"✅ [{i}/{len(pdf_paths)}] 완료: report_id={report.get('report_id')}"
                    )
                else:
                    failed += 1
                ...
                failed += 1
                reports.append(
                    {"status": "error", "pdf_path": pdf_path, "error": str(e)}
                )
