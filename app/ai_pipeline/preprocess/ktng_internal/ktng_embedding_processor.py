...
class KTNGEmbeddingProcessor:
    ...
    def __init__(self, collection_name: str = "remon_internal_ktng", reset_collection: bool = False):
        """
        초기화.
        ...
        self.local_client = VectorClient(collection_name=collection_name, use_local=True)
        ...
        logger.info(f"✅ KTNG 임베딩 프로세서 초기화: collection={collection_name} (이중 저장 모드, reset={reset_collection})")
    
    async def process_and_store(self, combined_chunks: List[Dict[str, Any]], source_file: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i, chunk in enumerate(combined_chunks):
                ...
                metadata.update({
                    ...
                    # 소스 파일 정보
                    "meta_source_file": source_file.split("/")[-1] if source_file else "json_cases"
                })
            ...
            logger.info(f"✅ KTNG 임베딩 처리 완료: {len(combined_chunks)}개 청크 이중 저장 (Docker + 로컬)")
