...
class PreprocessOrchestrator:
    ...
    def process_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("  📖 정의 추출 중...")
            definition_results = (
                self.definition_extractor.extract_definitions_and_acronyms(raw_text)
            )
            ...
            try:
                ...
                if (
                    qdrant_ready_data
                    and "sparse_embedding" in qdrant_ready_data[0]["metadata"]
                ):
                    sparse_embeddings = [
                        d["metadata"].pop("sparse_embedding") for d in qdrant_ready_data
                    ]
                ...
                logger.info(
                    f"  ✅ Qdrant 이중 저장 완료: {len(texts)}개 청크 (Docker + 로컬)"
                )
    ...
    def _prepare_for_qdrant_with_chunks(
        ...
        for chunk_idx, (chunk, propositions) in enumerate(
            zip(chunks, all_propositions)
        ):
            chunk_id = f"{doc_metadata.get('doc_id', 'unknown')}_chunk_{chunk_idx}"

            # Dense embedding
            dense_emb = (
                embeddings_array[chunk_idx].tolist()
                if hasattr(embeddings_array[chunk_idx], "tolist")
                else embeddings_array[chunk_idx]
            )
            ...
            chroma_doc = {
                ...
                "metadata": {
                    ...
                    "meta_cfr_citation": (
                        legal_hierarchy.get("full_citation")
                        if legal_hierarchy
                        else None
                    ),
                    "meta_regulation_hierarchy": (
                        legal_hierarchy.get("regulation_type")
                        if legal_hierarchy
                        else None
                    ),
                },
        ...
        logger.debug(
            f"Qdrant 저장용 데이터 준비: {len(chroma_data)}개 청크 (중 Sparse: {sparse_count}개)"
        )
        return chroma_data
    ...
    def process_regulatory_document_ddh(
        self, document_text: str, document_metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            doc_id = document_metadata.get(
                "doc_id", self._generate_doc_id("ddh_document")
            )
            ...
            for chunk in processed_chunks:
                chunk.text = definition_extractor_enhanced.normalize_tobacco_terms(
                    chunk.text
                )
                chunk.context_text = (
                    definition_extractor_enhanced.normalize_tobacco_terms(
                        chunk.context_text
                    )
                )
            ...
            child_texts = [
                chunk.text for chunk in processed_chunks if chunk.chunk_type == "child"
            ]
            parent_texts = [
                chunk.text for chunk in processed_chunks if chunk.chunk_type == "parent"
            ]

            # Child 임베딩 (Dense + Sparse)
            child_embeddings_result = (
                self.embedding_pipeline.embed_texts(child_texts)
                if child_texts
                else {"dense": [], "sparse": []}
            )
            if child_texts and child_embeddings_result.get("sparse"):
                logger.info(
                    f"  ✅ Child Sparse 벡터: {len(child_embeddings_result['sparse'])}개"
                )

            # Parent 임베딩 (Dense + Sparse)
            parent_embeddings_result = (
                self.embedding_pipeline.embed_texts(parent_texts)
                if parent_texts
                else {"dense": [], "sparse": []}
            )
            if parent_texts and parent_embeddings_result.get("sparse"):
                logger.info(
                    f"  ✅ Parent Sparse 벡터: {len(parent_embeddings_result['sparse'])}개"
                )
            ...
            for chunk in processed_chunks:
                if chunk.chunk_type == "child":
                    # Dense 임베딩
                    chunk.embedding = (
                        child_embeddings_result["dense"][child_idx].tolist()
                        if hasattr(
                            child_embeddings_result["dense"][child_idx], "tolist"
                        )
                        else child_embeddings_result["dense"][child_idx]
                    )
                    # Sparse 임베딩 (있으면)
                    if child_embeddings_result.get("sparse"):
                        chunk.sparse_embedding = child_embeddings_result["sparse"][
                            child_idx
                        ]
                    child_idx += 1
                elif chunk.chunk_type == "parent":
                    # Dense 임베딩
                    chunk.parent_embedding = (
                        parent_embeddings_result["dense"][parent_idx].tolist()
                        if hasattr(
                            parent_embeddings_result["dense"][parent_idx], "tolist"
                        )
                        else parent_embeddings_result["dense"][parent_idx]
                    )
                    # Sparse 임베딩 (있으면)
                    if parent_embeddings_result.get("sparse"):
                        chunk.parent_sparse_embedding = parent_embeddings_result[
                            "sparse"
                        ][parent_idx]
                    parent_idx += 1
            ...
            result = {
                ...
                "parent_child_chunking": {
                    ...
                    "parent_chunks": len(
                        [c for c in processed_chunks if c.chunk_type == "parent"]
                    ),
                    "child_chunks": len(
                        [c for c in processed_chunks if c.chunk_type == "child"]
                    ),
                },
                "embeddings": {
                    ...
                    "embedding_dim": (
                        len(processed_chunks[0].embedding)
                        if processed_chunks and processed_chunks[0].embedding
                        else 1024
                    ),
                },
                "qdrant_ready_data": qdrant_ready_data,
                "summary": {
                    ...
                    "normalized_terms": len(
                        definition_extractor_enhanced.get_all_terms()
                    ),
                },
            }

            logger.info(
                f"✅ DDH 처리 완료: {len(legal_nodes)}개 섹션 → {len(processed_chunks)}개 Parent-Child 청크"
            )
            return result
    ...
    def _prepare_parent_child_for_qdrant(
        ...
        for chunk in processed_chunks:
            ...
            sparse_embedding = (
                ...
                if chunk.chunk_type == "child"
                else chunk.parent_sparse_embedding
            )
            if sparse_embedding:
                sparse_count += 1
            ...
            qdrant_doc = {
                ...
                "metadata": {
                    ...
                    "context_text": (
                        chunk.context_text[:500] + "..."
                        if len(chunk.context_text) > 500
                        else chunk.context_text
                    ),
                    # 기본 메타데이터
        ...
        logger.debug(
            f"Qdrant Parent-Child 데이터 준비: {len(qdrant_data)}개 (중 Sparse: {sparse_count}개)"
        )
        return qdrant_data
