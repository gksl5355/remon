...
client_openai = OpenAI(
    api_key=os.getenv("OPENAI_API_KEY"), http_client=httpx.Client(trust_env=False)
)
...
async def score_impact_node(state: AppState) -> AppState:
    ...
    if state.get("refined_score_impact_prompt"):
        ...
        if (
            "Force impact_level to 'High'" in refined_prompt
            or "FORCE IMPACT_LEVEL TO 'HIGH'" in refined_prompt.upper()
        ):
            impact_level = "High"
            weighted_score = 4.5
            logger.info("[Impact] ✅ HITL override: Force High level (4.5)")
        elif (
            "Force impact_level to 'Medium'" in refined_prompt
            or "FORCE IMPACT_LEVEL TO 'MEDIUM'" in refined_prompt.upper()
        ):
            impact_level = "Medium"
            weighted_score = 3.0
            logger.info("[Impact] ✅ HITL override: Force Medium level (3.0)")
        elif (
            "Force impact_level to 'Low'" in refined_prompt
            or "FORCE IMPACT_LEVEL TO 'LOW'" in refined_prompt.upper()
        ):
            impact_level = "Low"
            weighted_score = 2.0
            ...
            impact_level = (
                "High"
                if weighted_score >= 4
                else "Medium" if weighted_score >= 2.5 else "Low"
            )
            logger.warning(
                f"[Impact] ⚠️ HITL prompt 감지 실패, 기본 로직 사용: {refined_prompt[:100]}..."
            )
    ...
    if state.get("refined_score_impact_prompt"):
        if "reasoning to 'Human in the loop'" in state["refined_score_impact_prompt"]:
            ...
            logger.info(
                "[Impact] ✅ HITL override: reasoning set to 'Human in the loop'"
            )
    ...
    logger.info(
        "[Impact] Final Impact Score: %s (Level: %s, Score: %.2f)",
        impact_item,
        impact_level,
        weighted_score,
    )
    ...
    if regulation_id and state.get("impact_scores"):
        ...
        async with AsyncSessionLocal() as session:
            ...
            try:
                ...
                logger.info(
                    f"✅ 영향도 중간 결과물 저장 완료: regulation_id={regulation_id}"
                )
