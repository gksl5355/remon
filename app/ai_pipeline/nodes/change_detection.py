...
class ConfidenceScorer:
    ...
    def adjust_confidence(result: Dict[str, Any]) -> float:
        ...
        if "adversarial_check" in result:
            base_confidence = result["adversarial_check"].get("adjusted_confidence", base_confidence)
    ...
    @staticmethod
    def get_confidence_level(confidence: float) -> Literal["HIGH", "MEDIUM", "LOW", "UNCERTAIN"]:
        if confidence >= 0.9:
            return "HIGH"
        elif confidence >= 0.7:
            return "MEDIUM"
...
class ChangeDetectionNode:
    ...
    async def run(self, state: AppState) -> AppState:
        ...
        if not change_context:
            ...
            state["change_summary"] = {"status": "skipped", "reason": "no_change_context"}
            return state
        ...
        if legacy_vision_results:
            ...
            # S3 검색 모드
            if not legacy_regulation_id:
                legacy_regulation_id = await self._find_legacy_regulation_s3(vision_results)
                if not legacy_regulation_id:
            ...
            legacy_ref_blocks = await self._get_legacy_reference_blocks_s3(legacy_regulation_id)
            if not legacy_ref_blocks:
                logger.warning(f"Legacy Reference Blocks 조회 실패: {legacy_regulation_id}")
                state["change_detection_results"] = []
    ...
    def _extract_reference_blocks(self, vision_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _find_legacy_regulation_s3(self, vision_results: List[Dict[str, Any]]) -> Optional[str]:
        ...
        logger.info(f"S3 Legacy 검색: title={title}, country={country}, type={regulation_type}")
    ...
    # async def _find_legacy_regulation(self, vision_results: List[Dict[str, Any]]) -> Optional[str]:
        ...
        logger.info(f"Legacy 검색: title={title}, country={country}, type={regulation_type}")
        ...
        try:
            ...
            if results.get("metadatas"):
                for idx, meta in enumerate(results["metadatas"]):
                    ...
                    score = results["scores"][idx] if idx < len(results["scores"]) else 0
                    
                    if legacy_id and score > 0.7:  # 유사도 임계값
                        logger.info(f"Legacy 규제 발견: {legacy_id} (score: {score:.2f})")
                        return legacy_id
            ...
            if keywords:
                ...
                if results2.get("metadatas"):
                    ...
                    logger.info(f"Keywords로 Legacy 발견: {legacy_id} (score: {score:.2f})")
                    return legacy_id
    ...
    async def _get_legacy_reference_blocks_s3(self, s3_key: str) -> List[Dict[str, Any]]:
    ...
    async def _match_reference_blocks(
        ...
        for new_block in new_blocks:
            ...
            # 동일 Section 번호 찾기
            legacy_block = next(
                (b for b in legacy_blocks if b["section_ref"] == new_section),
                None
            )
    ...
    async def _detect_change_by_ref_id(
        ...
        # Reference ID 생성
        new_ref_id = f"{new_regulation_id}-{section_ref}-P{new_block.get('page_num', 0)}"
        legacy_ref_id = f"{legacy_regulation_id}-{section_ref}-P{legacy_block.get('page_num', 0)}"
