...
class MappingNode:
    ...
    async def _classify_change_requirement(
        ...
        prompt = {
            ...
            "instructions": (
                "제품 스펙 목록에서 매칭되는 feature를 찾거나, 없으면 new_requirement로 표시.\n"
                "출력: {\"match_status\": \"existing_feature\"|\"new_requirement\"|\"ambiguous\","
                "\"matched_feature\": \"name or null\", \"reason\": \"string\", \"suggested_hint\": \"string or null\"}"
            ),
        }
        async with sem:
            try:
